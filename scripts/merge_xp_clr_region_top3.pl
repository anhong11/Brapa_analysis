...
$top = int(0.03 * $number);
my $threshod_3 = int($sorted[$top]);
$top = int(0.04 * $number);
my $threshod_4 = int($sorted[$top]);
$top = int(0.1 * $number);
...
print "$first\t$threshod_3\t$threshod_4\t$threshod_5\t$threshod_10\t$threshod_20\t$threshod_50\n";
...
foreach my $chr (sort {$a <=> $b} @chr_array) {
	foreach my $group (sort {$a <=> $b} keys %{$merged{$chr}}) {
		...
		my $int_max_score = int($max_score+0.5);
		if ($int_max_score >= $threshod_3) {
